"""
Stage 2 - Adding Pydantic Validation

Builds on Stage 1 by introducing a `Patient` Pydantic model. This gives us:
    - Automatic request validation (bad age, wrong gender, etc. are rejected
        with a 422 before our code even runs).
    - Auto-generated, accurate OpenAPI docs at /docs.
    - A `verdict` field computed from height/weight, so the BMI category is
        always derived from the data instead of being typed in by hand.

Still missing (added in Stage 3):
    - Update and delete endpoints -> this stage can only create and read.
...
class Patient(BaseModel):
    ...
    `verdict` is a computed field: it is never supplied by the client and is
    always recalculated from `height` and `weight` whenever the model is
    built or serialized, so it can never go stale.
    """
    ...
    gender: Annotated[
        Literal["male", "female", "other"], Field(..., title="Patient's gender")
    ]
...
app = FastAPI(title="Patient Management API - Stage 2 (Pydantic Validation)")
...
def view_patient(
    patient_id: str = Path(..., description="The ID of the patient to look up", examples=["P001"])
):
...
def add_patient(patient: Patient):
    ...
    # Store everything except the ID itself, which is already the dict key.
